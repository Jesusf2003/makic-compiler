...
type_t *PR_FindType(type_t *type)
{
    ...
    {
        if (check->type != type->type || check->aux_type != type->aux_type || check->num_parms != type->num_parms)
            continue;
...
type_t *PR_ParseType(void)
{
    ...
    if (PR_Check("."))
    {
        ...
        newer.type = ev_field;
        newer.aux_type = PR_ParseType();
        return PR_FindType(&newer);
    }
    ...
    // function type
    memset(&newer, 0, sizeof(newer));
    newer.type = ev_function;
    newer.aux_type = type; // return type
    newer.num_parms = 0;
    if (!PR_Check(")"))
    {
        if (PR_Check("..."))
            newer.num_parms = -1; // variable args
        else
            do
            {
                type = PR_ParseType();
                name = PR_ParseName();
                strcpy(pr_parm_names[newer.num_parms], name);
                newer.parm_types[newer.num_parms] = type;
                newer.num_parms++;
            } while (PR_Check(","));
